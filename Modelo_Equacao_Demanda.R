...
####################### CARREGAR PACOTES ####################################################
library(ggplot2)       # viasualizacao grcafica 
...
library(ggpubr)        # inserir equacao no grafico
library(gganimate)     # graficos animados
library(ggthemes)      # adicionar novos temas
library(plotly)        # grafico dinamicos
...
library(scatterplot3d) # grafico 3d
...
setwd('C:/Users/mario Dhiego/Documents/Modelo_Equacao_Demanda')
getwd()
list.files()
###############################################################################################

##################### LEITURA DE BANCO DE DADOS #############################################
# Dados Demanda de Produtos Alimentícios

demanda <- read_excel('Demanda_Ufra.xlsx')
...
# Constru???O do Modelo
Modelo1 <- lm(formula = Qpeixe~Ppeixe+Pacai+Pfar+Pcboi+Pfrango+Renda+Educa??o+IPSustent, data= demanda)
Modelo1

Modelo2 <- lm(formula = Qpeixe~Ppeixe+Pacai+Pfar+Pcboi+Pfrango+Renda+Educa??o, data= demanda)
Modelo2

Modelo3 <- lm(formula = Qpeixe~Ppeixe+Pacai+Pfar+Pcboi+Renda+Educa??o, data= demanda)
Modelo3

Modelo4 <- lm(formula = Qpeixe~Ppeixe+Pacai+Pfar+Pcboi+Renda, data= demanda)
Modelo4

Modelo5 <- lm(formula = Qpeixe~Ppeixe+Pacai+Pcboi+Renda, data= demanda)
Modelo5

Modelo6 <- lm(formula = Qpeixe~Ppeixe++Pcboi+Renda, data= demanda)
Modelo6



Previsao1 <- predict(Modelo6, data.frame(Ppeixe=7,Pcboi= 13.10, Renda= 3350), se=T)
Previsao1
#########################################################################################
...
###### Analise de Residuos ##############################################################
par(mfrow=c(2,2))
plot(Modelo6,  which = 4)


########################################################################################


####### Normalidade dos riduos #######################################################
shapiro.test(Modelo6$residuals)

which(rstudent(Modelo6) > 2)
...
rstandard(Modelo6)
summary(rstandard(Modelo6))
plot(rstandard(Modelo6))
#######################################################################################
...
####### Homocedasticidade dos residuos (Breusch-Pagan) ####################################
bptest(Modelo6)

library(car)

leveneTest(Qpeixe ~ Ppeixe*Pboi, demanda, center = mean)
leveneTest(Modelo6$residuals)

##########################################################################################

####### Diagrama de Dispersao com Equacao da Reta ########################################

Plot2 <- ggplot(demanda, aes(y=Qpeixe, x=Ppeixe))+
  geom_point(size=2.5, pch=21, col='black',fill='coral')+
  geom_smooth(formula = y~x, method = 'lm', se=TRUE)+
  stat_regline_equation(aes(label= paste(..eq.label.., ..adj.rr.label..,
                        sep = "*plain(\",\")~~")),
                        label.x = 0, label.y = 500, size=6.0, color='black')+
  theme_bw()+
...
########################################################################################
###### Ausancia de Multicolinearidade

###### Fator de Inflacao da Variancia (VIF)
pairs.panels(demanda)

vif(Modelo6)

vif(Modelo1,Modelo2,Modelo3,Modelo4,Modelo5,Modelo6)
#######################################################################################


###### Verificar Significancia (p-valor) ##############################################
summary(Modelo1)
summary(Modelo2)
summary(Modelo3)
summary(Modelo4)
...
graph <- scatterplot3d(demanda$Qpeixe~demanda$Ppeixe+demanda$Pcboi+demanda$Renda,
                       pch= 16, color ="darkblue", angle=55, grid=TRUE,box=TRUE,
                       highlight.3d=FALSE, col.grid = "lightblue",
                      xlab="Preço Peixe", ylab="Quantidade Peixe", zlab="Preço da Carne Boi",
                      main = "Demanda do Consumidor de Produtos Aliment?cios de Belém",
                      sub = "Equação de Demanda")

graph$plane3d(Modelo6, col="black", type = "h", 
...
###### Giro Interativo no gr?fico
library(plot3D)
library(plot3Drgl)
library(scatterplot3d)

###### Exemplo Usaremos o conjunto de dados da iris 
data(iris)
head(iris)
